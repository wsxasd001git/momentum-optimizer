...
(function($) {
    ...
    let settings = {
        ...
        hysteresisThreshold: 10 // Порог гистерезиса в %
    };
    ...
    function bindEvents() {
        ...
        $('#ms-hysteresis').on('input', function() {
            settings.hysteresisThreshold = parseInt($(this).val());
            $('#ms-hysteresis-value').text(settings.hysteresisThreshold);
            debouncedRecalculate();
        });
     ...
     */
    function calcMomentumAtIndex(i, useHysteresis) {
        ...
        let targetN = settings.topN;

        if (settings.dynamicMode) {
            const marketVol = calcMarketVol(i);

            // Плавное масштабирование: чем выше волатильность, тем меньше акций
            const volRatio = settings.marketVolThreshold / Math.max(marketVol, 5);
            targetN = Math.round(settings.topN * Math.pow(volRatio, 0.5));
            targetN = Math.max(5, Math.min(targetN, 30)); // Ограничения

            if (useHysteresis && typeof window.lastAdjustedTopN !== 'undefined') {
                // Гистерезис: меняем только при существенном отклонении
                const changePercent = Math.abs(targetN - window.lastAdjustedTopN) / window.lastAdjustedTopN;

                if (changePercent > settings.hysteresisThreshold / 100) {
                    adjustedTopN = targetN;
                    window.lastAdjustedTopN = adjustedTopN; // Обновляем для следующей итерации
                } else {
                    adjustedTopN = window.lastAdjustedTopN; // Держим прежнее значение
                }
            } else {
                adjustedTopN = targetN;
            }
        ...
        return {
            ...
            targetN: targetN, // Целевой размер без гистерезиса
            date: currentDate
        };
    }
    ...
    function recalculate() {
        ...
        window.lastAdjustedTopN = settings.topN; // Для гистерезиса
